...
#define EXAMPLE_GIF_PLAYER_TASK_STACK_SIZE (8 * 1024)
#define EXAMPLE_GIF_PLAYER_TASK_PRIORITY   3
...
void app_main(void)
{
    ...
    uart0_init();
    uart2_init();
    ESP_ERROR_CHECK(i2c_slave_drv_init());
    ...
    if (sd_ok) {
        if (gif_player_init(panel_handle, io_handle, EXAMPLE_LCD_H_RES, EXAMPLE_LCD_V_RES)) {
            BaseType_t gif_task_ok = xTaskCreate(gif_player_task, "GIF", EXAMPLE_GIF_PLAYER_TASK_STACK_SIZE,
                                                 NULL, EXAMPLE_GIF_PLAYER_TASK_PRIORITY, NULL);
            if (gif_task_ok != pdPASS) {
                ESP_LOGE(TAG, "Failed to create GIF player task");
            }
        }
    } else {
        ESP_LOGW(TAG, "SD card not mounted. Cannot play GIF.");
